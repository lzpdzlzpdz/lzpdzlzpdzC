 ...
 * @member:	the name of the list_struct within the struct.
 */
#define list_for_each_entry_safe_ext(pos, pos_type,n, head, member)			\
	for (pos = list_entry((head)->next, pos_type, member),	\
...
/*GNU C*/
#define hlist_for_each_entry_safe(entry, pos, n, head, member) 		 \
	for (pos = (head)->first;					 \
	     pos && ({ n = pos->next; 1; }) && 				 \
		({ entry = hlist_entry(pos, typeof(*entry), member); 1;}); \
	     pos = n)
